...
const Badge = ({ children }: { children: React.ReactNode }) => (
  <span className="inline-flex items-center rounded-full bg-amber-50 dark:bg-amber-900/30 px-2.5 py-0.5 text-xs font-medium text-amber-700 dark:text-amber-300 ring-1 ring-inset ring-amber-200 dark:ring-amber-700">
    {children}
  </span>
);
...
const DetailModal = ({
  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/40 dark:bg-black/60 backdrop-blur-sm flex items-center justify-center p-4">
      <div className="w-full max-w-2xl rounded-2xl bg-white dark:bg-gray-800 shadow-xl overflow-hidden transition-colors">
        <div className="relative">
          <img src={item.image} alt={item.name} className="h-72 w-full object-cover" />
          <button
            onClick={onClose}
            aria-label="Tutup detail"
            className="absolute right-3 top-3 rounded-full bg-white dark:bg-gray-800 shadow p-2 text-gray-600 dark:text-gray-300 hover:bg-amber-50 dark:hover:bg-amber-900/30 hover:text-amber-600 dark:hover:text-amber-500 transition"
          >
            <X className="h-5 w-5" />
          </button>
        </div>
  ...
  return (
    <section className="py-12 bg-white dark:bg-gray-900 transition-colors">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <h1 className="text-3xl font-bold mb-6 text-gray-900 dark:text-white">Motif Explorer</h1>
        <div className="flex flex-wrap gap-3 mb-6">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="border border-gray-300 dark:border-gray-600 px-3 py-2 rounded-lg pl-10 w-full 
          ...
          <select
            ...
            className="border border-gray-300 dark:border-gray-600 px-3 py-2 rounded-lg cursor-pointer
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {filtered.length > 0 ? (
              filtered.map((item) => (
                <button
                  ...
                  className="border border-gray-200 dark:border-gray-700 rounded-xl overflow-hidden shadow-sm 
                    ...
                    hover:shadow-md hover:border-amber-300 dark:hover:border-amber-600 transition text-left 
              ...
              <div className="col-span-full text-center py-12">
                <p className="text-gray-500 dark:text-gray-400">Tidak ada motif ditemukan</p>
              </div>
            )}
          </div>
        )}
